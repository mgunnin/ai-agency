...
export class Calc extends Component<CalcProps, CalcState> {
  private cursor: number | null = null;
  private terminal_rows: number = 2;
  ...
  componentDidUpdate() {
    clearInterval(this.cursor || undefined);
  ...
  focusCursor = (e: React.MouseEvent) => {
    clearInterval(this.cursor || undefined);
    this.startCursor($(e.target).data("row-id"));
  }

  unFocusCursor = (e: React.FocusEvent) => {
  ...
  checkKey = (e: React.KeyboardEvent) => {
    if (e.key === "Enter") {
      ...
      let command = $(`input#calculator-input-${terminal_row_id}`).val();
  ...
  evaluteExp = (command: string): string => {
    let result = "";
    let expr: Expression;
    try {
      expr = parser.parse(command);
      try {
        result = parser.evaluate(command, this.variables).toString();
        if ((expr as any).tokens && (expr as any).tokens.length === 2 && (expr as any).tokens[2]?.type === "IOP2") {
          this.variables[expr.variables()[0]] = parseFloat(result);
        }
      } catch (e) {
