...
export class Terminal extends Component<TerminalProps, TerminalState> {
  ...
  private terminal_rows: number;
  private current_directory: string;
  private curr_dir_name: string;
  private prev_commands: string[];
  private commands_index: number;
  private child_directories: { [key: string]: string[] };
  ...
  constructor(props: TerminalProps) {
    ...
    this.child_directories = {
      ...
      "case-studies": [
        "AI-Driven-Marketing-Optimization.pdf",
        "Predictive-Maintenance-in-Manufacturing.pdf",
        "Customer-Sentiment-Analysis.pdf",
        "Fraud-Detection-in-Finance.pdf",
      ],
      services: [
        "AI-Consulting",
      ...
      team: [
        "Matt-Gunnin-CEO",
        "Raquel-Ortega-COO",
        "Jim-Huang-Lead-Data-Scientist",
        "Timoty-Jones-Head-of-Engineering",
      ],
      technologies: [
        "Python",
        "TensorFlow",
      ...
      clients: ["TechCorp", "HealthPlus", "FinBank", "RetailGiant", "EduWorld"],
  ...
  terminalRow = (id: number): JSX.Element => {
    return (
      <React.Fragment key={id}>
        <div className="flex w-full h-5">
          <div className="flex">
            <div className=" text-ubt-green">Admin@VerticalLabs</div>
  ...
  focusCursor = (e: React.MouseEvent<HTMLDivElement>) => {
    clearInterval(this.cursor)
    this.startCursor($(e.target).data("row-id"))
  }

  unFocusCursor = (e: React.FocusEvent<HTMLInputElement>) => {
  ...
  checkKey = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      ...
      let command = $(`input#terminal-input-${terminal_row_id}`).val() as string
  ...
  handleCommands = (command: string, rowId: number) => {
    let words = command.trim().split(" ").filter(Boolean)
    let main = words[0]
    words.shift()
    let result = ""
    let rest = words.join(" ")
    ...
    switch (main) {
      ...
      case "ls":
        ...
        } else if (target === "company-documents") {
          result = "Nope! 🙃"
          break
        } else {
      ...
      case "pwd":
        ...
        result = str.replace("~", "/home/vertical-labs")
        break
      case "code":
        if (words[0] === "." || words.length === 0) {
          ...
          result =
            "Command '" +
            main +
            "' not found, or not yet implemented.<br>Available Commands:[ cd, ls, pwd, echo, clear, exit, mkdir, code, spotify, chrome, about-vertical-labs, todoist, trash, settings, sendmsg]"
        }
        break
      case "echo":
      ...
      case "trash":
        ...
        break
      case "about-vertical-labs":
        if (words[0] === "." || words.length === 0) {
          this.props.openApp("about-vertical-labs")
        } else {
          result =
            "Command '" +
            main +
...
export const displayTerminal = (addFolder: (name: string) => void, openApp: (id: string) => void): JSX.Element => {
  return (
