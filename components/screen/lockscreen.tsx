...
const LockScreen: React.FC<LockScreenProps> = ({ isLocked, bgImgName, unLockScreen }) => {
  const wallpapers: WallpaperRecord = {
  ...
  return (
    <div
      ...
      style={{ zIndex: 100 }}
      className={`absolute outline-none bg-black bg-opacity-90 transform duration-500 select-none top-0 right-0 overflow-hidden m-0 p-0 h-screen w-screen ${isLocked ? "visible translate-y-0" : "invisible -translate-y-full"
        }`}
    >
      <div
        style={{
